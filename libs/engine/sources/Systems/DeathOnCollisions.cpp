...
void System::DeathOnCollisions::operator()()
{
    for (auto &collision : _events.getEvents<event::Collision>()) {
        if (_killOnCollisions[collision.entity]
            and _solids[collision.secondEntity]
            and not isAlreadyInDeathEvent(collision.entity)) {
            _events.addEvent<event::Death>(collision.entity);
        }
    }
}

bool System::DeathOnCollisions::isAlreadyInDeathEvent(engine::Entity entity
) const
{
